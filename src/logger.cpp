...
void logger::_private::process_entry(const logger::type& type, const std::string& msg,
		const bool& entry_only, const std::string& detailTxt, const std::string& tagTxt) {
	...
	if ( _entry.equals(_last)) {
		...
		_entry.tag = _entry.tag.empty() && tagTxt != "\x1B" ? _last.tag : _entry.tag;
	...
	if ( !_last2_idx )
		...
		it -> count++;
		it -> tag = tagTxt == "\x1B" ? "" : ( _entry.tag.empty() ? it -> tag : _entry.tag);
...
void logger::_private::flush(const logger::type& type) {
	...
	logger::_private::_tag[type].clear();
...
bool logger::_private::endOfEntry(const logger::type& f) {
	...
	if ( logger::_private::_stream[f].str().find_first_of('\n', 0) !=
		...
		std::string str;
		std::string t = logger::_private::_tag[f];
		std::string d = logger::_private::_detail[f];
		...
		logger::_private::process_entry(f, str, just_entry, d, t);
		return true;
	}
...
const std::vector<logger::entry> logger::last(const int& count, const logger::type& type) {
